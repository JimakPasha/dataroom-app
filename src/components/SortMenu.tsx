...
import { setSortBy, setSortDirection, setFoldersPosition } from '@/store/settingsSlice';
...
export const SortMenu = () => {
  ...
  const { sortBy, sortDirection, foldersPosition } = useAppSelector((state) => state.settings);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuItem 
          onClick={() => dispatch(setFoldersPosition('top'))}
          className={cn(foldersPosition === 'top' && 'bg-accent')}
        >
          <Check className={cn('h-4 w-4 mr-2', foldersPosition !== 'top' && 'invisible')} />
          Folders on top
        </DropdownMenuItem>
        <DropdownMenuItem 
          onClick={() => dispatch(setFoldersPosition('mixed'))}
          className={cn(foldersPosition === 'mixed' && 'bg-accent')}
        >
          <Check className={cn('h-4 w-4 mr-2', foldersPosition !== 'mixed' && 'invisible')} />
          Mixed with files
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
